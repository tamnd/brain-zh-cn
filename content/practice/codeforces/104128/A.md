...
[CF 104128A - 停止，昨天请不要再说了](https://codeforces.com/problemset/problem/104128/A)
...
因此$Q^*$满足定理 X 中的条件 (120)，即大子集之间存在匹配$P$和$Q$在中间位互补的情况下。 
...
(c) 部分使用补体配对${0,\dots,2^{l-1}-1}$，以及约束$g(p)+g(q)=2^{l-1}$强制执行条件（120）所需的完美匹配结构。 

(d) 部分根据限制条件下的密度保持得出$Q''$和单射性$g$。 

## 注释

 该结构是通用散列的标准两级应用：首先压缩为$l$位同时控制碰撞，然后使用中间进行细化$l-1$用于诱导残基模配对图的位$2^{l-1}$。 奇数乘数条件为$a$确保截断在高位块和中位块之间保留足够的独立性，这对于单射性步骤至关重要。
