...
date: "2026-07-02T22:03:25+07:00"
...
[CF 103648E - 观鸟](https://codeforces.com/problemset/problem/103648/E)
...
= (-1)^{\sum_{j \ge 0} b_j} \prod_{j \ge 0} r_{j+1}(x)^{b_j}.$$让$\nu(k) = \sum_{j \ge 0} b_j$是$1$- 二进制展开中的位$k$。 然后$$w_k(1-x) = (-1)^{\nu(k)} w_k(x).$$### 与声称的身份比较
 ...
 要测试的语句是$$w_k(-x) = (-1)^k w_k(x).$$由于沃尔什函数通常定义为$[0,1)$并定期延长，$-x$对应于$1-x$在这个设置下。 导出的恒等式表明正确的指数取决于汉明权重$\nu(k)$，不在$k$本身。 
