...
[CF 103260M - 离散对数是个笑话](https://codeforces.com/problemset/problem/103260/M)
...
每个单项式$u \in A_{t+1}$可以写成$x_i m$最多在$\deg_{x_i}(u)$方式，因此最多$t+1$总计方式，但更重要的是最多固定数量，仅取决于$s$以及学位的结构-$t$单项式。 第 7.2.1.3 节中的定理 M 确定了均匀压缩常数$\kappa_s$使得影子算子满足下界$$|\partial A_t| \ge N_t + \kappa_s N_t.$$自从$\partial A_t \subseteq A_{t+1}$，我们得到$$N_{t+1} \ge |\partial A_t| \ge N_t + \kappa_s N_t,$$这产生$$N_{t+1} \ge N_t + \kappa_s N_t.$$这样就完成了(b)的证明。 ∎
