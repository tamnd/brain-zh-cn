...
[CF 103107A - 和 RMQ](https://codeforces.com/problemset/problem/103107/A)
...
从$m \le x$和单调性$f$，由此可知$$\binom{m}{t-1} \le \binom{x}{t-1}.$$替换定义给出$$\kappa_t^{(\mathbb{Z})}(N) \le \kappa_t^{(\mathbb{R})}(N).$$如果$x$是一个整数，那么必然$x=m$，所以等式成立。 
