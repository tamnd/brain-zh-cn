...
[CF 104550C - 日志记录](https://codeforces.com/problemset/problem/104550/C)
...
一个更结构化的粗暴想法是模拟剥离：计算凸包，将其删除，计算下一个包，等等。 这是有效的，因为每个“层”对应一个移除步骤。 然而，每次删除后从头开始重新计算船体的成本很高。 如果天真地完成，每个船体计算成本为 O(N log N)，并且在最坏的情况下可能有 O(N) 层，导致 O(N² log N)。 
